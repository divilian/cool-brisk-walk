...
\framebox{\includegraphics[width=0.5\textwidth]{Prim9.png}}
...
\index{descendent (of a node)}
\item[descendent.] Your children, grandchildren, great-grandchildren,
\textit{etc.}, all the way the leaves. B's descendents are C, D and E,
...
own right! F itself is a tree with only one node. B and its descendents
...
\begin{enumerate}
\item Visit the root.
\item Treat the left child and all its descendents as a subtree, and
...
\begin{enumerate}
\item \label{inorder:left} Treat the left child and all its descendents as
...
descendent of Mitch's \textit{left} subtree, whereas she must properly be
...
node with \textit{the left-most descendent of its right subtree}. (Or,
equivalently, the right-most descendent of its left subtree).
...
the right child, but because \textit{Jim had no left descendents.} If he
had, promoting him would have been just as wrong as promoting Ben. Instead,
we would have promoted \textit{Jim's left-most descendent.}
...
Because she was the left-most descendent of Mitch's right subtree.
